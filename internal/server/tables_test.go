...
func TestTablesHandlers_ListTablesHandler(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			buildStubs: func(repo *mock.MockRepository) {
				repo.EXPECT().ListTables().Times(1).Return([]dto.PragmaticTableWithID{}, nil)
			},
			checkResponse: func(t *testing.T, recorder *httptest.ResponseRecorder) {
	...
	for i := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			server.listTablesHandler(recorder, req)
